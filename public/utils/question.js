...
class Question extends Phaser.Scene {
  ...
  getTime(now) {
    ...
    now.timer = now.add
      .text(
        ...
        "TIME:" + this.timeLife,
        { fontStyle: "bolder", fontSize: "25.2px", fill: "#FFF" }
      )
      .setOrigin(0.5);
    var more = this;
  ...
  getPoint(now) {
    ...
    var scoreText = "";
    this.turn = this.turn == "1PLAYER" ? "2PLAYER" : "1PLAYER";

    if (ENV.VERSUS == "2P") {
      if (this.turn == "1PLAYER") {
        this.point1++;
        scoreText = "SCORE:" + this.point1 + "  " + this.turn;
      } else {
        this.point2++;
        scoreText = "SCORE:" + this.point2 + "  " + this.turn;
      }
    } else {
      this.point++;
      scoreText = "SCORE:" + this.point;
    }
    ...
    now.score = now.add
      .text(
        (modalX + modalWidth / 2) * (ENV.VERSUS == "2P" ? 0.69 : 0.4),
        (modalY + modalHeight / 2) * 0.15,
        scoreText,
        { fontStyle: "bolder", fontSize: "25.2px", fill: "#FFF" }
      )
  ...
  mathQuestion() {
    ...
    a =
      ENV.LEVEL == "E"
        ? Phaser.Math.RND.between(2, 9)
        : ENV.LEVEL == "N"
        ? Phaser.Math.RND.between(4, 11)
        : Phaser.Math.RND.between(7, 17);
    b =
      ENV.LEVEL == "E"
        ? Phaser.Math.RND.between(2, 9)
        : ENV.LEVEL == "N"
    ...
    c =
      (ENV.LEVEL == "H" || ENV.LEVEL == "N") && randomOp1 == "/"
        ? Phaser.Math.RND.between(1.7, 1.9)
        : Phaser.Math.RND.between(3, 5);
    ...
    return this.intNumber(d) == false && ENV.LEVEL == "E"
      ? this.mathQuestion()
      : (res = {
          a,
          b,
  ...
  customQuestion() {
    ...
    return q.a == ENV.LEVEL && q.b == this.rootPage
      ? (res = {
          a: q.a,
          b: q.b,
          d: q.d,
